...
class ReadCSV (threading.Thread):
    def __init__(self, threadID, queue, args, df):
    ...
    def test_csv_header(self, csv_columns, must_exist_columns):
    ...
    def accepted_obs(self, cell, date, speciekey, lat, lon):
        global big_data, nobs_accepted
    ...
    def main(self):
        ...
        global big_data
        global nobs_accepted
        global nobs_rejected
        global nobs_repeated
        global nobs_outside_grid_or_time
        global nobs_wrong_format
        global nobs
        global usecols
        global f
        ...
        for index, row in df.iterrows():
            ...
            self.test_csv_header(csv_columns, usecols)
            '''
            ...
            try:
                ...
                #print(self.threadID, index, gbifid, speciekey, lon, lat, date)
                ...
                if grid_maker.is_in_grid([lon, lat]) and date_obj >= args.strdate and date_obj <= args.enddate and (not args.species or speciekey in args.species):
                    ...
                    try:
                        if speciekey in big_data[cell]['dates'][date]:
                            ...
                            self.accepted_obs(cell, date, speciekey, lat, lon)
        ...
        #elapsed_time(s0_time)

    def run(self):
        queue = self.queue
        self.main()
        queue.get()
        if args.vverbose: print('     finish thread: ' + str(self.threadID))
...
def get_parser():
    ...
    if max_threads >= 4:
        ...
        if max_threads == 1:
            max_threads = 2
        choices = list(range(1, max_threads))
    parser.add_argument('-t', dest='nthreads', type=int, choices=choices, help='use multi-threading. Number of thread', default=2, required=False)
...
if __name__ == "__main__":
    ...
    workQueue = queue.Queue(maxsize=args.nthreads) ## 0-4
    ...
    for df in pd.read_csv(args.input, sep=args.separator, chunksize=chunksize, engine='c', low_memory=False, usecols=usecols, skip_blank_lines=True):
        ...
        if args.vverbose: print('     start thread ' + str(df_id))
        thread = ReadCSV(df_id, workQueue, args, df)
        thread.start()
        thread.setName(df_id)
    ...
    for process in threads:
        #print('     waiting thread to complete: ' + process.getName())
        process.join()
